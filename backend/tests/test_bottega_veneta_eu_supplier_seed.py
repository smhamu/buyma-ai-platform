...
from scripts.seed_bottega_veneta_eu_suppliers import (
    BOTTEGA_VENETA_EU_SUPPLIERS,
    CONSERVATIVE_POLICY,
    seed_bottega_veneta_eu_suppliers,
)
...
def test_seed_has_four_unique_eur_country_suppliers():
    assert {item["country_code"] for item in BOTTEGA_VENETA_EU_SUPPLIERS} == {
        "FR", "DE", "IT", "ES"
    }
    assert len({item["name"] for item in BOTTEGA_VENETA_EU_SUPPLIERS}) == 4
    assert len({item["website_url"] for item in BOTTEGA_VENETA_EU_SUPPLIERS}) == 4
...
@pytest.mark.asyncio
async def test_seed_relates_new_suppliers_to_existing_bottega_veneta_brand():
    brand = Brand(id=uuid4(), brand_code="bottega_veneta", brand_name="Bottega Veneta", luxury_tier="luxury", online_purchase_policy="unknown", is_research_enabled=True, is_active=True)
    session = make_session(brand=brand)
    assert await seed_bottega_veneta_eu_suppliers(session, uuid4()) == 4
...
async def test_seed_is_idempotent_and_does_not_overwrite_existing_suppliers():
    existing = [SimpleNamespace(name=item["name"], notes="operator-owned") for item in BOTTEGA_VENETA_EU_SUPPLIERS]
    session = make_session(brand=SimpleNamespace(id=uuid4()), existing=existing)
    assert await seed_bottega_veneta_eu_suppliers(session, uuid4()) == 0
...
@pytest.mark.asyncio
async def test_seed_fails_closed_when_bottega_veneta_brand_is_missing():
    session = make_session(brand=None)
    with pytest.raises(RuntimeError, match="Bottega Veneta brand is missing"):
        await seed_bottega_veneta_eu_suppliers(session, uuid4())
