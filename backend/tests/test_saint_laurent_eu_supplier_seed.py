...
from scripts.seed_saint_laurent_eu_suppliers import (
    CONSERVATIVE_POLICY,
    SAINT_LAURENT_EU_SUPPLIERS,
    seed_saint_laurent_eu_suppliers,
)
...
def test_seed_has_four_unique_eur_country_suppliers():
    assert {item["country_code"] for item in SAINT_LAURENT_EU_SUPPLIERS} == {"FR", "DE", "IT", "ES"}
    assert len({item["name"] for item in SAINT_LAURENT_EU_SUPPLIERS}) == 4
    assert len({item["website_url"] for item in SAINT_LAURENT_EU_SUPPLIERS}) == 4
...
@pytest.mark.asyncio
async def test_seed_relates_new_suppliers_to_existing_saint_laurent_brand():
    brand = Brand(id=uuid4(), brand_code="saint_laurent", brand_name="Saint Laurent", luxury_tier="luxury", online_purchase_policy="unknown", is_research_enabled=True, is_active=True)
    session = make_session(brand=brand)
    assert await seed_saint_laurent_eu_suppliers(session, uuid4()) == 4
...
async def test_seed_is_idempotent_and_does_not_overwrite_existing_suppliers():
    existing = [SimpleNamespace(name=item["name"], notes="operator-owned") for item in SAINT_LAURENT_EU_SUPPLIERS]
    session = make_session(brand=SimpleNamespace(id=uuid4()), existing=existing)
    assert await seed_saint_laurent_eu_suppliers(session, uuid4()) == 0
...
@pytest.mark.asyncio
async def test_seed_fails_closed_when_saint_laurent_brand_is_missing():
    session = make_session(brand=None)
    with pytest.raises(RuntimeError, match="Saint Laurent brand is missing"):
        await seed_saint_laurent_eu_suppliers(session, uuid4())
