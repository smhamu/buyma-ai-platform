...
async def list_suppliers(
    ...
    suppliers = await service.repository.find_all_filtered(
        owner_user_id=None if current_user.role == "admin" else current_user.id,
        country=country, ships_to_japan=ships_to_japan,
        buyma_allowed_status=buyma_allowed_status, research_status=research_status,
        is_active=is_active,
    )
    return success_response(data=[SupplierResponse.model_validate(item) for item in suppliers], message="Suppliers fetched successfully.")
