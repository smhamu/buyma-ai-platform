...
test("supplier policy review moves through no evidence, inconsistent, up to date and review due",async({page})=>{
  ...
  try{
    ...
    await evidencePanel.getByRole("button",{name:"Close"}).click();await page.getByRole("link",{name:"Policy Review",exact:true}).click();await expect(page.getByRole("row").filter({hasText:supplierName})).toContainText("Inconsistent");
    ...
    await page.getByRole("link",{name:"Policy Review",exact:true}).click();await expect(page.getByRole("row").filter({hasText:supplierName})).toContainText("Up To Date");
