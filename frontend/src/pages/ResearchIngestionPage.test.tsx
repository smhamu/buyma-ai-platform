...
describe("ResearchIngestionPage",()=>{
  ...
  it("registers a URL with an optional purchase restriction without implying an automatic fetch",async()=>{vi.mocked(registerResearchUrl).mockResolvedValue({id:"source-1"} as never);render(<MemoryRouter><ResearchIngestionPage/></MemoryRouter>);fireEvent.click(await screen.findByRole("button",{name:"Add URL"}));fireEvent.change(screen.getByRole("combobox",{name:"Supplier"}),{target:{value:supplier.id}});expect(screen.getByText(/Automated fetching disabled/)).toBeVisible();fireEvent.change(screen.getByRole("combobox",{name:"Purchase restriction (optional)"}),{target:{value:"client_advisor_only"}});fireEvent.change(screen.getByRole("textbox",{name:"Product URL"}),{target:{value:"https://shop.example/item/1"}});fireEvent.click(screen.getByRole("button",{name:"Register URL"}));await waitFor(()=>expect(registerResearchUrl).toHaveBeenCalledWith(supplier.id,"https://shop.example/item/1",null,"client_advisor_only"));});
